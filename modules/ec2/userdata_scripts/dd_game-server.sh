...
# the script is templated by terraform and meant to be executed as a userdata script for dedicated server (chat server) instances 
set -x
...
aws s3 cp $S3_RESOURCE_BUCKET/setup_dd.sh /home/$SERVICE_USERNAME/setup_dd.sh
aws s3 cp $S3_RESOURCE_BUCKET/status_dd.sh /home/$SERVICE_USERNAME/status_dd.sh
aws s3 cp $S3_RESOURCE_BUCKET/metacity_dd_server.service /etc/systemd/system/
chown $SERVICE_USERNAME:$SERVICE_USERNAME /etc/systemd/system/metacity_dd_server.service
chown -R $SERVICE_USERNAME:$SERVICE_USERNAME /home/$SERVICE_USERNAME
chmod +x /home/$SERVICE_USERNAME/setup_dd.sh
chmod +x /home/$SERVICE_USERNAME/status_dd.sh
# start backend server systemd unit
systemctl daemon-reload
systemctl enable metacity_dd_server.service
systemctl start metacity_dd_server.service
# check service and update status table
bash -c "/home/$SERVICE_USERNAME/status_dd.sh $BUILD_VERSION $TABLE_NAME"