module "auth_service_alb" {
  source = "../../../../../modules/alb"

  certificate_arn                = data.terraform_remote_state.landing-zone.outputs.wildcard_dev_cert_arn
  dns_name                       = "auth.${local.env_short_names[var.env]}.${lower("${var.project_name}")}.${var.root_domain_public_fqdn}"
  env                            = var.env
...
module "auth_service" {
  ...
  service_config = {
    alb_target_group_arn = "${module.auth_service_alb.target_group_arn}"
    ...
    execution_role_arn      = "${data.terraform_remote_state.metacity-core.outputs.ecs_task_execution_role_arn}" #external state dependency
    network_mode            = "awsvpc"
    service_registry_arn    = "${data.terraform_remote_state.backend-cluster.outputs.private_service_dns_namespace_arn}" #ecs cluster outputs
    sg_ids                  = ["${data.terraform_remote_state.metacity-core.outputs.auth_service_ecs_tasks_sg_id}"]      #external state dependency
    subnet_ids              = "${data.terraform_remote_state.metacity-core.outputs.public_subnet_ids}"                   #external state dependency
    task_family             = "${var.service_name}"
    namespace_id            = "${data.terraform_remote_state.backend-cluster.outputs.private_service_dns_namespace_id}" #external state dependency
  }
}
